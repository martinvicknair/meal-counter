...
this.events.publish(e+".initialized");return this}var e="jSignature",y=function(a,f){var b;this.kick=function(){clearTimeout(b);b=setTimeout(f,a)};
this.clear=function(){clearTimeout(b)};return this},t=function(a){this.topics={};
this.context=a?a:this;
this.publish=function(a,b,c,d){if(this.topics[a]){var g=this.topics[a],j=Array.prototype.slice.call(arguments,1),i=[],k,e,h,n;e=0;for(h=g.length;e<h;e++)n=g[e],k=n[0],n[1]&&(n[0]=function(){},i.push(e)),k.apply(this.context,j);e=0;
for(h=i.length;e<h;e++)g.splice(i[e],1)}};
this.subscribe=function(a,b,c){this.topics[a]?this.topics[a].push([b,c]):this.topics[a]=[[b,c]];return{topic:a,callback:b}};
this.unsubscribe=function(a){if(this.topics[a.topic])for(var b=this.topics[a.topic],c=0,d=b.length;
	c<d;
	c++)b[c][0]===a.callback&&b.splice(c,1)}},z=function(a){var f=this.canvasContext,b=a.x[0],a=a.y[0],c=this.settings.lineWidth,d=f.fillStyle;f.fillStyle=f.strokeStyle;
	f.fillRect(b+c/-2,a+c/-2,c,c);
	f.fillStyle=d},u=function(a,f){var b=new h(a.x[f-1],a.y[f-1]),c=new h(a.x[f],

a.y[f]),d=b.getVectorToPoint(c);if(1<f){var g=new h(a.x[f-2],a.y[f-2]),j=g.getVectorToPoint(b),i;if(j.getLength()>this.lineCurveThreshold){i=2<f?(new h(a.x[f-3],a.y[f-3])).getVectorToPoint(g):new m(0,0);
		var e=0.35*j.getLength(),k=j.angleTo(i.reverse()),n=d.angleTo(j.reverse());i=(new m(i.x+j.x,i.y+j.y)).resizeTo(Math.max(0.05,k)*e);var l=(new m(j.x+d.x,j.y+d.y)).reverse().resizeTo(Math.max(0.05,n)*e),j=this.canvasContext,e=g.x,n=g.y,k=b.x,q=b.y,w=g.x+i.x,g=g.y+i.y;i=b.x+l.x;l=b.y+l.y;j.beginPath();
j.moveTo(e,n);j.bezierCurveTo(w,g,i,l,k,q);j.stroke()}}d.getLength()<=this.lineCurveThreshold&&(d=this.canvasContext,g=b.x,b=b.y,i=c.x,c=c.y,d.beginPath(),d.moveTo(g,b),d.lineTo(i,c),d.stroke())},v=function(a){var b=a.x.length-1;
	if(0<b){var c=new h(a.x[b],a.y[b]),d=new h(a.x[b-1],a.y[b-1]),g=d.getVectorToPoint(c);
	if(g.getLength()>this.lineCurveThreshold){if(1<b){var a=(new h(a.x[b-2],a.y[b-2])).getVectorToPoint(d),j=(new m(a.x+g.x,a.y+g.y)).resizeTo(g.getLength()/2),g=this.canvasContext,a=d.x,b=d.y,
...
p.prototype.resetCanvas=function(a){
    var b=this.canvas,c=this.settings,d=this.canvasContext,g=this.isCanvasEmulator,j=b.width,i=b.height;
    d.clearRect(0,0,j+30,i+30);
    d.shadowColor=d.fillStyle="white"; // changed this line to work with jsPDF
    d.fillRect(0,0,j+30,i+30);         //changed this line to work with jsPDF
    d.lineWidth=Math.ceil(parseInt(c.lineWidth,10));
    d.lineCap=d.lineJoin="round";
    
d.strokeStyle=c["decor-color"];d.shadowOffsetX=0;d.shadowOffsetY=0;var k=Math.round(i/5),n=1.5*k,h=i-k,j=j-1.5*k,i=i-k;d.beginPath();d.moveTo(n,h);d.lineTo(j,i);d.stroke();d.strokeStyle=c.color;g||(d.shadowColor=d.strokeStyle,d.shadowOffsetX=0.5*d.lineWidth,d.shadowOffsetY=-0.6*d.lineWidth,d.shadowBlur=0);a||(a=[]);d=this.dataEngine=new s(a,this,z,u,v);c.data=a;$(b).data(e+".data",a).data(e+".settings",c);var l=this.$parent,q=this.events,w=e;d.changed=function(){q.publish(w+".change");
...
Math.round(c.height()*a.sizeRatio));c.addClass(e);b.width=c.width();b.height=c.height();a=b;

if(a.getContext)a=!1;else if("undefined"!==typeof FlashCanvas)a=FlashCanvas.initElement(a),c=1,window&&(window.screen&&window.screen.deviceXDPI&&window.screen.logicalXDPI)&&(c=1*window.screen.deviceXDPI/window.screen.logicalXDPI),1!==c&&($(a).children("object").get(0).resize(Math.ceil(a.width*c),Math.ceil(a.height*c)),a.getContext("2d").scale(c,c)),a=!0;else throw Error("Canvas element does not support 2d context. jSignature cannot proceed.");
this.isCanvasEmulator=a;b.onselectstart=function(a){a&&a.preventDefault&&a.preventDefault();a&&a.stopPropagation&&a.stopPropagation();return false};return b};var k=function(a,c){var b=new Image,d=this;

b.onload=function(){d.getContext("2d").drawImage(b,0,0,b.width<d.width?b.width:d.width,b.height<d.height?b.height:d.height)};
b.src="data:"+c+","+a},q=function(a){this.find("canvas."+e).add(this.filter("canvas."+e)).data(e+".this").resetCanvas(a);return this},b=function(a,b){if(void 0===b&&("string"===
